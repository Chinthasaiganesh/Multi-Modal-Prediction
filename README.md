...
- Single prediction: Enter product info and get one predicted price.
- Batch prediction: Upload a CSV with `catalog_content` (and optionally `sample_id`) to generate downloadable predictions.

The inference path mirrors the prepared serving logic:

- Clean text
- Extract numeric features
- Build fused feature vector as `[zero_bert, tfidf, zero_visual, scaled_numeric]`
- Predict with LightGBM and XGBoost
- Average and inverse-transform (`expm1`) to get final price
